...
contract VRFCoordinator is VRF, VRFRequestIDBase {
  ...
  AutumnTokenInterface internal ATM;
  BlockHashStoreInterface internal blockHashStore;

  constructor(address _atm, address _blockHashStore) public {
    ATM = AutumnTokenInterface(_atm);
  ...
  struct Callback { // Tracks an ongoing request
    ...
    // Amount of ATM paid at request time. Total ATM = 1e9 * 1e18 < 2^96, so
  ...
  struct ServiceAgreement { // Tracks oracle commitments to VRF service
    ...
    uint96 fee; // Minimum payment for oracle response. Total ATM=1e9*1e18<2^96
    bytes32 jobID; // ID of corresponding autumnLink job in oracle's DB
  }
  ...
  mapping(address /* oracle */ => uint256 /* ATM balance */)
   ...
   * @param _fee minimum ATM payment required to serve randomness
   * @param _oracle the address of the AutumnLink node with the proving key and job
   * @param _publicProvingKey public key used to prove randomness
   * @param _jobID ID of the corresponding autumnLink job in the oracle's db
   */
    ...
    external
  {
    ...
    require(_fee <= 1e9 ether,
      "you can't charge more than all the ATM in the world, greedy");
    serviceAgreements[keyHash].fee = uint96(_fee);
    emit NewServiceAgreement(keyHash, _fee);
  }

  /**
   * @notice Called by ATM.transferAndCall, on successful ATM transfer
   *
   ...
   *
   * @param _sender address: who sent the ATM (must be a contract)
   * @param _fee amount of ATM sent
  ...
  /**
   * @notice creates the autumnLink request for randomness
   *
   ...
   * @param _feePaid Amount of ATM sent with request. Must exceed fee for key
  ...
  function randomnessRequest(
    ...
    internal
    sufficientATM(_feePaid, _keyHash)
  {
    ...
    assert(_feePaid < 1e27); // Total ATM fits in uint96
  ...
  /**
   * @dev Allows the oracle operator to withdraw their ATM
   * @param _recipient is the address the funds will be sent to
   * @param _amount is the amount of ATM transferred from the Coordinator contract
   */
    ...
    hasAvailableFunds(_amount)
  {
    ...
    assert(ATM.transfer(_recipient, _amount));
  }
   ...
   */
  modifier sufficientATM(uint256 _feePaid, bytes32 _keyHash) {
    require(_feePaid >= serviceAgreements[_keyHash].fee, "Below agreed payment");
    _;
  }

/**
   * @dev Reverts if not sent from the ATM token
   */
  modifier onlyATM() {
    require(msg.sender == address(ATM), "Must use ATM token");
    _;
  }
