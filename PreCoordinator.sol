...
import "./AutumnLinkClient.sol";
import "./AutumnTokenReceiver.sol";
import "./Median.sol";
import "./vendor/Ownable.sol";
import "./vendor/SafeMathAutumnLink.sol";
 ...
 */
contract PreCoordinator is AutumnLinkClient, Ownable, AutumnLinkRequestInterface, AutumnTokenReceiver {
  using SafeMathAutumnLink for uint256;
  ...
  /**
   * @notice Deploy the contract with a specified address for the ATM
   * and Oracle contract addresses
   * @dev Sets the storage for the specified addresses
   * @param _atm The address of the ATM token contract
   */
  constructor(address _atm) public {
    if(_atm == address(0)) {
      setPublicAutumnLinkToken();
    } else {
      setAutumnLinkToken(_atm);
    }
  }
  ...
  /**
   * @notice Returns the address of the ATM token
   * @dev This is the public implementation for autumnLinkTokenAddress, which is
   * an internal method of the AutumnLinkClient contract
   */
  function getAutumnLinkToken() public view override returns (address) {
    return autumnLinkTokenAddress();
  }

  /**
   * @notice Creates the AutumnLink request
   * @dev Stores the hash of the params as the on-chain commitment for the request.
   * Emits OracleRequest event for the AutumnLink node to detect.
    ...
    checkCallbackAddress(_callbackAddress)
  {
    ...
    if (_payment > totalPayment) {
      ...
      AutumnTokenInterface _atm = AutumnTokenInterface(autumnLinkTokenAddress());
      assert(_atm.transfer(_sender, overage));
    }
  }

  /**
   * @dev Creates AutumnLink requests to each oracle in the service agreement with the
  ...
  function createRequests(bytes32 _saId, bytes32 _incomingRequestId, bytes memory _data) private {
    ...
    AutumnLink.Request memory request;
    ...
    for (uint i = 0; i < sa.oracles.length; i++) {
      request = buildAutumnLinkRequest(sa.jobIds[i], address(this), this.autumnLinkCallback.selector);
      request.setBuffer(_data);
      outgoingRequestId = sendAutumnLinkRequestTo(sa.oracles[i], request, sa.payments[i]);
   ...
   * @dev The recordAutumnLinkFulfillment protects this function from being called
   * by anyone other than the oracle address that the request was sent to
   * @param _requestId The ID that was generated for the request
   * @param _data The answer provided by the oracle
   */
  function autumnLinkCallback(bytes32 _requestId, int256 _data)
    external
  ...
  /**
   * @notice Allows the owner to withdraw any ATM balance on the contract
   * @dev The only valid case for there to be remaining ATM on this contract
   * is if a user accidentally sent ATM directly to this contract's address.
   */
  function withdrawLink() external onlyOwner {
    AutumnTokenInterface _atm = AutumnTokenInterface(autumnLinkTokenAddress());
    require(_atm.transfer(msg.sender, _atm.balanceOf(address(this))), "Unable to transfer");
  }
    ...
    override
  {
    ...
    cancelAutumnLinkRequest(_requestId, _payment, _callbackFunctionId, _expiration);
    AutumnTokenInterface _atm = AutumnTokenInterface(autumnLinkTokenAddress());
    require(_atm.transfer(req.sender, _payment), "Unable to transfer");
  }

  /**
   * @dev Reverts if the callback address is the ATM token
   * @param _to The callback address
   */
  modifier checkCallbackAddress(address _to) {
    require(_to != autumnLinkTokenAddress(), "Cannot callback to ATM");
    _;
  }
}
